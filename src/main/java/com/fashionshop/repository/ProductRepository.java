...
public interface ProductRepository extends JpaRepository<Product, Long> {
	// TÃ¬m sáº£n pháº©m theo danh má»¥c cha (Ä‘á»‡ quy)
	// Query nÃ y khÃ¡ phá»©c táº¡p vÃ¬ product ná»‘i vá»›i category con, ta cáº§n tÃ¬m theo
	// category cha
	...
	@Query("SELECT DISTINCT p FROM Product p " +
			...
			"AND (c.id = :categoryId OR cp.id = :categoryId OR cpp.id = :categoryId) " +
			"ORDER BY p.createdAt DESC")
	List<Product> findTop10ByCategoryId(@Param("categoryId") Long categoryId, Pageable pageable);

	// HÃ€M Váº N NÄ‚NG (Ä\ufffdÃƒ NÃ‚NG Cáº¤P): Chá»‰ hiá»‡n sáº£n pháº©m cÃ³ Ã­t nháº¥t 1 mÃ u Ä‘ang Active
	@Query("SELECT DISTINCT p FROM Product p " +
	        "JOIN p.productColors pc " + 
			"LEFT JOIN pc.variants v " + 
