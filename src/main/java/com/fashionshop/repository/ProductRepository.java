...
public interface ProductRepository extends JpaRepository<Product, Long> {
	...
	@Query("SELECT DISTINCT p FROM Product p " + "JOIN p.category c " + "JOIN p.productColors pc " + // Join để check
																										// màu
	...
	@Query("SELECT DISTINCT p FROM Product p " + "JOIN p.productColors pc " + // 1. Đổi LEFT JOIN -> JOIN (Bắt buộc SP
																				// phải có màu)
			"LEFT JOIN pc.variants v " + // Vẫn giữ LEFT JOIN cho variant (phòng khi chưa nhập size)
			"LEFT JOIN p.category cat " + "WHERE pc.isActive = true " + // 2. QUAN TRỌNG: Chỉ lấy những dòng có màu đang
																		// Active
			"AND (:keyword IS NULL OR p.name LIKE %:keyword%) "
			+ "AND (:categoryId IS NULL OR cat.id = :categoryId OR cat.parent.id = :categoryId OR cat.parent.parent.id = :categoryId) "
			+ "AND (:sizes IS NULL OR v.size.name IN :sizes) " + "AND (:colors IS NULL OR pc.color.name IN :colors) "
			+ "AND (:minPrice IS NULL OR p.basePrice >= :minPrice) "
			+ "AND (:maxPrice IS NULL OR p.basePrice <= :maxPrice)")
	Page<Product> findProductsWithFilters(@Param("keyword") String keyword, @Param("categoryId") Long categoryId,
